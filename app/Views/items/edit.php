...
<section class="content">
    ...
    <div class="box-body">
        <?php if (!empty(session()->getFlashdata('error'))) : ?>
            ...
            <?php endif; ?>
        <form method="post" action="<?= base_url('items/update/' . $items->id_items) ;?>">
        <div class="row">
            <div class="col-md-4 col-md-offset-4">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="id_items" id="id_items" value="<?= $items->id_items; ?>" class="form-control" required>
                    </div>
                    <div class="form-group">
                        <!-- penggunaan "for" disini agar ketika klik label, maka akan fokus ke "id" tujuan, yaitu disini pada input product name -->
                        <label for="product_name">Name Items</label>
                        <input type="text" name="name" id="name" value="<?= $items->name; ?>" class="form-control" required>
                    </div>
                   
                    <div class="form-group">
                        <label>Price *</label>
                        <input type="text" name="price" id="price" value="<?= $items->price; ?>" class="form-control" required>
                    </div>
                    <div class="form-group">
                        <!-- cara lain untuk menampilkan data dari tabel lain, menggunakan form helper dari codeigniter -->
                        <label>Stock *</label>
                        <input type="text" name="stock" id="stock" value="<?= $items->stock; ?>" class="form-control" required>
                    </div>
                    <div class="form-group">
